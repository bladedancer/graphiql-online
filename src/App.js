...
const endpoint = 'https://gmatthews.dev.ampc.axwaytest.net/apis/graphql' // Initial
...
export default class App extends React.Component {
  constructor (props) {
    ...
    this.state = {
      ...
      endpoint,
      authorization,
      fetcher: this.createFetcher(authorization, endpoint)
    }
  }
  ...
  */
  createFetcher = (auth, endpoint) => param => fetch(
    endpoint,
    {
      ...options,
      headers: auth ? { ...options.headers, Authorization: `Bearer ${auth}` } : { ...options.headers },
  ...
  changeEndpoint = endpoint => this.setState({
    endpoint,
    fetcher: this.createFetcher(this.state.authorization, endpoint)
  })

  /**
   * Change auth and fetcher.
   */
  changeAuthorization = authorization => this.setState({
    authorization,
    fetcher: this.createFetcher(authorization, this.state.endpoint)
  })
   ...
   */
  enterToken = () => this.changeAuthorization(window.prompt('Choose the authorization token', this.state.authorization))
  ...
  render () {
    return (
      <div id='container'>
        ...
        >
          <GraphiQL.Logo>
            <a href='https://github.com/bladedancer/graphiql-online' title='See GraphiQL Online on GitHub'>
